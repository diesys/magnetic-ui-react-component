 ...
 */
export type ModeType = 'attract' | 'repel';
...
function zeroVelocity(): AnimVelocity {
  return { tx: 0, ty: 0, rotate: 0, skewX: 0, skewY: 0, scaleX: 0, scaleY: 0, scale: 0, rotateX: 0, rotateY: 0, opacity: 0 };
}
 ...
 */
function computeTarget(dx: number, dy: number, d: number, props: MagneticProps): AnimState {
  const {
    radius = 150,
    falloff = 'quadratic',
    mode = 'attract',
    ...
    tilt = false,      tiltIntensity = 15,
    opacity: opacityEnabled = false, opacityIntensity = 0.4,
  } = props;
  ...
  const tx = move ? sign * nx * force * moveIntensity : 0;
  const ty = move ? sign * ny * force * moveIntensity : 0;
    ...
    const animate = () => {
      ...
      if (el) {
        ...
        if (props.disabled) {
          ...
          target     = computeTarget(dx, dy, d, props);
        }
        ...
        if (props.spring) {
          ...
          // No spring: snap directly to target every frame
          const keys = Object.keys(cur) as (keyof AnimState)[];
